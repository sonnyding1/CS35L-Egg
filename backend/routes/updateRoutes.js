...
router.post("/user/file", async(req, res) => {
    try{
        ...
        const updateFields = {};
        if (fileName && fileName !== file.fileName) updateFields.fileName = fileName;
        if (folder && folder !== file.folder) updateFields.folder = folder;
        if (typeof public === 'boolean' && public !== file.public) updateFields.public = public;
        if (description && description !== file.description) updateFields.description = description;
...
router.post("/user/comment", async(req, res) =>{
    try{
        ...
        const comment = await Comment.findOne({_id, authorId: req.session.userId});
        if (!comment) {
            return res
                .status(StatusCodes.NOT_FOUND)
